...
export function ProfileCard({ perfil }) {
    ...
    const nomeExibicao = perfil.nome || perfil.login;
    const dataFormatada = new Date(perfil.criado_em_github).toLocaleDateString('pt-BR');
    const repositorios = perfil.repositorios || [];
    ...
    return (
        <Card className="shadow mt-4 border-0" style={{ borderRadius: '15px' }}>
            <Card.Header className="text-center bg-white border-0 pt-4" style={{ borderTopLeftRadius: '15px', borderTopRightRadius: '15px' }}>
                ...
                />
                <Card.Title className="fs-3 fw-bold">{nomeExibicao}</Card.Title>
            ...
            <Card.Body>
                <div className="d-flex justify-content-center gap-2 mb-4 mt-2 flex-wrap">
                    <Badge bg="primary" className="p-2 shadow-sm rounded-pill px-3">Seguidores: {perfil.seguidores}</Badge>
                    <Badge bg="secondary" className="p-2 shadow-sm rounded-pill px-3">Seguindo: {perfil.seguindo}</Badge>
                    <Badge bg="dark" className="p-2 shadow-sm rounded-pill px-3">Repositórios: {perfil.repositorios_publicos}</Badge>
                </div>
                        ...
                        repositorios.map(repo => (
                            <ListGroup.Item key={repo.id} className="d-flex justify-content-between align-items-center p-3">
                                <div className="ms-2 me-auto">
                                    <div className="fw-bold text-dark">{repo.nome}</div>
                                    <small className="text-muted">{repo.linguagem || 'Sem linguagem definida'}</small>
                                </div>
                                <Button variant="outline-dark" size="sm" href={repo.url} target="_blank" className="rounded-pill px-3">
                                    Ver código
                                </Button>
                            </ListGroup.Item>
                        ))
